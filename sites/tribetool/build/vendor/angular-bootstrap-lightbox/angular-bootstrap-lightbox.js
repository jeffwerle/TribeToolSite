  ...
  $templateCache.put('lightbox.html',
    '<div class="modal-body" ng-swipe-left="Lightbox.nextImage()" ng-swipe-right="Lightbox.prevImage()">' +
        "<div class=lightbox-nav>" +
            "<modal-close-button on-cancel=\"$dismiss()\"></modal-close-button>" +
            "<div class=btn-group>" +
                "<a ng-show=\"Lightbox.images.length > 1\" class=\"btn btn-xs btn-default\" ng-click=Lightbox.prevImage()>‹ Previous</a> <a ng-href={{Lightbox.imageUrl}} target=_blank class=\"btn btn-xs btn-default\" title=\"Open In New Tab\">Open Image In New Tab</a> <a ng-show=\"Lightbox.images.length > 1\" class=\"btn btn-xs btn-default\" ng-click=Lightbox.nextImage()>Next ›</a>" +
            "</div>" +
        "</div>" +
    ...
    $templateCache.put('lightbox.html',
        '<div class="modal-body">' +
            "<div class=lightbox-nav>" +
                ...
                "<div class=btn-group>" +
                    "<a ng-show=\"Lightbox.images.length > 1\" class=\"btn btn-xs btn-default\" ng-click=prevImage()>‹ Previous</a> <a ng-href={{Lightbox.imageUrl}} target=_blank class=\"btn btn-xs btn-default\" title=\"Open In New Tab\">Open Image In New Tab</a> <a ng-show=\"Lightbox.images.length > 1\" class=\"btn btn-xs btn-default\" ng-click=nextImage()>Next ›</a>" +
                "</div>" +
                '<div ng-show="Lightbox.isImageOwner" style="clear: both;">' +
                    '<a class="btn btn-xs btn-default" ng-click="editImage()">Edit Album</a>' +
                "</div>" +
            "</div>" +
            '<div class=lightbox-image-container>' +
                '<div class=lightbox-image-caption>' +
                    '<span>{{Lightbox.imageCaption}}</span>' +
                '</div>' +
                '<div ng-show="Lightbox.isLoading"><loading></loading></div>' +

                // ng-swipe-left="nextImage()" ng-swipe-right="prevImage()"
                '<img interactive-image class="centered" ng-class="{\'pointer\': Lightbox.images.length > 1}" ng-click="imageClicked()" lightbox-src="{{Lightbox.imageUrl}}" alt="">' +
            "</div>" +

            '<div ng-if="Lightbox.imageFileEntry">' +

                '<div ng-show="Lightbox.loadingComments"><loading></loading> Loading Comments...</div>' +
                '<div ng-if="!Lightbox.loadingComments">' +
                    '<div ng-show="Lightbox.imageFileEntry.TagText" class="centered"><div btf-markdown="Lightbox.imageFileEntry.TagText"></div></div>' +
                    '<div style="margin-top: 10px; margin-bottom: 10px;">' +
                        '<div>' +
                            '<div style="float: left">' +
                                '<emotion-vote-mechanism image-file-entry="Lightbox.imageFileEntry" emotion="Lightbox.imageFileEntry.RequesterEmotion"></emotion-vote-mechanism>' +
                            '</div>' +
                            '<div style="float: left">' +
                                '<div class="post-submitted-by-text"><interaction-summary votable="Lightbox.imageFileEntry"></interaction-summary></div>' +
                            '</div>' +
                        '</div>' +
                        '<div class="clearfix"></div>' +
                    '</div>' +
                    ...
                    '<div>' +
                        '<comment-input options="Lightbox.commentInputOptions" image-file-entry="Lightbox.imageFileEntry"></comment-input>' +
                    '</div>' +
                    '<div ng-if="Lightbox.imageFileEntry.Comments && Lightbox.imageFileEntry.Comments.length > 0" class="comment-layer">' +
                        '<div ng-repeat="comment in Lightbox.imageFileEntry.Comments">' +
                            '<comment-entry options="Lightbox.options" comment="comment"></comment-entry>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
            '</div>' +

            '<button class="btn btn-primary pull-right" style="clear: both; margin-top: 10px; margin-bottom: 10px; margin-right: 20px;" ng-click="cancel()" >Okay</button>' +
...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  /**
   * Template URL passed into `modalService.open()`.
   * @type     {String}
   * @name     templateUrl
   ...
   * @memberOf bootstrapLightbox.Lightbox
   */
  this.getImageUrl = function (options) {
      if(options.imageFileEntry && options.imageFileEntry.Full) {
          return options.imageFileEntry.Full.Url;
      }

    return options.image.url;
  };
  ...
  this.$get = ['$document', '$modal', '$timeout', 'cfpLoadingBar',
      'ImageLoader', 'modalService', 'navigationService', 'accountService', 'imageService', 'commService', '$routeParams', function ($document, $modal, $timeout, cfpLoadingBar,
      ImageLoader, modalService, navigationService, accountService, imageService, commService, $routeParams) {
    var Lightbox = {};
     ...
     * @param    {Array}  newImages An array of images. Each image may be {
     * url: string
     * caption: string
     * }
     * or {
    ...
    Lightbox.openModal = function (newImages, newIndex) {
        Lightbox.commentInputOptions = {
            onCancel: function() {

            }
        };
      Lightbox.isImageOwner = false;
      Lightbox.index = newIndex;
      Lightbox.setImages(newImages);

      // store the modal instance so we can close it manually if we need to
      Lightbox.modalInstance = modalService.open({
        'templateUrl': Lightbox.templateUrl,
        'controller': ['$scope', '$modalInstance', 'imageService', 'commService', 'modalService', '$routeParams', '$timeout', 'navigationService', 'accountService', 'profileService', function ($scope, $modalInstance, imageService, commService, modalService, $routeParams, $timeout, navigationService, accountService, profileService) {
          // $scope is the modal scope, a child of $rootScope
          $scope.Lightbox = Lightbox;

            Lightbox.$scope = $scope;
            ...
            $scope.editImage = function() {
                if(!$scope.isImageOwner) {
                    return;
                }

                imageService.selectPicture(function(image) {
                    // image selected
                },
                    {
                        getAlbumStack: function(onSuccess, onFailure, community) {
                            profileService.getAlbumStack(onSuccess, onFailure, {
                                Id: $scope.imageFileEntry.CommunityId
                            });
                        },
                        allowImageSizeSelection: false,
                        selectUploadedImage: false,
                        albumType: 'Any',
                        albumId: $scope.imageFileEntry.AlbumId
                    });
            };
/*
            $timeout(function() {
...
*/
        ...
        'windowClass': 'lightbox-modal',
          'backdrop': true
      }, /*onSuccess*/ null,/*onDismissed*/ null,
    ...
    Lightbox.setImage = function (newIndex) {
      ...
      Lightbox.index = newIndex;
        ...
        var scrollToCommentIfNecessary = function() {
            if($routeParams.comment) {
                $timeout(function() {
                    navigationService.scrollToComment($routeParams.comment);
                });
            }
        };

        var setImageFromUrl = function() {

            var success = function () {
                Lightbox.isLoading = false;
                Lightbox.options = Lightbox.images[Lightbox.index];
                if(Lightbox.options) {
                    Lightbox.image = Lightbox.options.image;
                }
            ...
            if(!imageUrl) {
                Lightbox.imageUrl = 'images/silhouette-medium.png';
            }
            else {
        ...
        var initializeImageData = function() {
            Lightbox.isImageOwner = Lightbox.imageFileEntry && accountService.isLoggedIn() && Lightbox.imageFileEntry.AccountId === accountService.account.Id && !Lightbox.imageFileEntry.TagPageId && !Lightbox.imageFileEntry.StepPageId && !Lightbox.imageFileEntry.MapId;
            if(Lightbox.imageFileEntry && !angular.isDefined(Lightbox.imageFileEntry.Comments)) {
                Lightbox.loadingComments = true;
                imageService.getImage(Lightbox.imageFileEntry, function(data) {
                    // Success
                    Lightbox.loadingComments = false;
                    Lightbox.imageFileEntry = data.ImageFileEntry;
                    Lightbox.options.imageFileEntry = Lightbox.imageFileEntry;
                    Lightbox.commentInputOptions.imageFileEntry = Lightbox.imageFileEntry;
                    setImageFromUrl();
        ...
        Lightbox.options = Lightbox.images[Lightbox.index];
        Lightbox.imageFileEntry = Lightbox.options.imageFileEntry;
        Lightbox.isImageOwner = false;
      var refreshImage = function() {
            ...
            if(!Lightbox.imageFileEntry && Lightbox.options.image && Lightbox.options.image.url) {
                // We don't have an image file entry--attempt to lookup the image by the url
                imageService.getImageByUrl(Lightbox.options.image.url, function(data) {
                    // Success
                    Lightbox.imageFileEntry = data.ImageFileEntry;
                    Lightbox.options.imageFileEntry = Lightbox.imageFileEntry;
                    Lightbox.commentInputOptions.imageFileEntry = Lightbox.imageFileEntry;
    ...
    Lightbox.setImages = function (newImages) {
        ...
        for(var i = 0; i < newImages.length; i++) {
            ...
            if(newImage.imageFileEntry) {
                newImage.image = {
                    url: newImage.imageFileEntry.Full.Url,
                    caption: newImage.imageFileEntry.Title
                };
                Lightbox.images.push(newImage);
            }
            else {
    ...
    'link': function (scope, element, attrs) {
      ...
      var resize = function () {
        ...
        element.css({
          ...
          if(modalDimensions.width < 650) {
              modalDimensions.width = 650;
          }
