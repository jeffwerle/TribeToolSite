 ...
 * ngImgCrop v0.3.2
 *
 * This is an edited version, implementing the "non-square selections" pull-request
 * https://github.com/alexk111/ngImgCrop/pull/1/files?diff=split
 * https://github.com/PortChaw/ngImgCrop
 *
 * It also has some features from
 * https://github.com/iblank/ngImgCrop
 * namely the "original-data" attribute
 *
...
(function() {
    ...
    crop.factory('cropAreaRectangle', ['cropArea', function(CropArea) {
        ...
        CropAreaRectangle.prototype.processMouseMove=function(mouseCurX, mouseCurY) {
            ...
            if (this._areaIsDragging) {
                ...
                // If we change to the the new center point, will it cause the
                // crop area to need to change? If so, we don't want to change it
                var newSize = {x: point.x - s.w / 2, y: point.y - s.h / 2, w: s.w, h: s.h};
                var boundedSize = this._preventBoundaryCollision(newSize);
                if(boundedSize.h !== s.h) {
                    newSize.y = s.y;
                    newSize.h = s.h;
                }
                if(boundedSize.w !== s.w) {
                    newSize.x = s.x;
                    newSize.w = s.w;
                }
        ...
        CropAreaRectangle.prototype.maximizeCrop = function(canvasElement) {
            if(!canvasElement)
                canvasElement = angular.element(this._ctx.canvas);
            // Maximize
            this.setSize({x: 0,
                y: 0,
        ...
        CropAreaRectangle.prototype.processMouseDoubleClick=function(mouseCurX, mouseCurY) {
            var canvasElement = angular.element(this._ctx.canvas);
            var size = this.getSize();
            if(this._isCoordWithinResizeCtrl([mouseCurX,mouseCurY])) {

                var canvasWidth = canvasElement.width();
                var canvasHeight = canvasElement.height();

                // Is the area already maximized?
                if(size.x === 0 && size.y === 0 && size.w === canvasWidth &&
                    size.h === canvasHeight) {
                    // minimize
                    this.setSize({x: 0,
                        y: 0,
                    ...
                    // Maximize
                    this.maximizeCrop(canvasElement);
                }
            }
        };
    ...
    crop.factory('cropArea', ['cropCanvas', function(CropCanvas) {
        ...
        CropArea.prototype.processMouseDoubleClick=function() {};
        ...
        return function(elCanvas, opts, events){
            ...
            var onMouseDoubleClick=function(e) {
                ...
                if(image!==null) {
                    ...
                    var pageX=e.pageX;
                    var pageY=e.pageY;
                    theArea.processMouseDoubleClick(pageX-offset.left, pageY-offset.top);
                    self.drawScene();
                }
            };
            ...
            this.getResultImageDataURIFullQuality =function() {
                return this.getResultImageDataURI(true);
            };

            this.getResultImageDataURI=function(fullQuality) {
                ...
                if(fullQuality) {
                    ...
                    var areaWidth = theArea.getSize().w;
                    var areaHeight = theArea.getSize().h;

                    var areaAdjustedWidth = areaWidth *(image.width/ctx.canvas.width);
                    var areaAdjustedHeight = areaHeight *(image.height/ctx.canvas.height);

                    temp_canvas.width = areaAdjustedWidth;
                    temp_canvas.height = areaAdjustedHeight;
                    if(image!==null){

                        // Determine the coordinates in relation to the image
            ...
            if(opts.allowCropping) {
                ...
                elCanvas.on('dblclick',onMouseDoubleClick);
                ...
                this.destroy=function() {
                    ...
                    elCanvas.off('dblclick',onMouseDoubleClick);
    ...
    crop.directive('imgCrop', ['$timeout', 'cropHost', 'cropPubSub', function($timeout, CropHost, CropPubSub) {
        return {
            ...
            link: function(scope, element/*, attrs*/) {
                ...
                if(!angular.isDefined(scope.allowCropping)) {
                    scope.allowCropping = true;
                }
                ...
                var cropHost=new CropHost(element.find('canvas'), {
                    allowCropping: scope.allowCropping
                }, events);

                // Store Result Image to check if it's changed
                var storedResultImage;

                var updateResultImageFromUri = function(scope, resultImage) {
                ...
                var updateResultImage=function(scope) {
                    ...
                    updateResultImageFromUri(scope, resultImage);
                    ...
                    if(scope.image !== ''){
                        ...
                        if(angular.isDefined(scope.originalData) && imgWidth !== 0 && imgHeight !== 0){
                            scope.originalData= {
                                ...
                                cropHost: cropHost,
                                events: events,
                                updateResultImage: function(resultImage) {
                                    updateResultImageFromUri(scope, resultImage);
                                }
                            };
                        }
